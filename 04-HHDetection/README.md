...
For this exercise you have to use two hash functions. Your update bloom filter option should do the following:
    ...
    2. Compute two hash functions using two different algorithms (e.g., `HashAlgorithm.crc16` and `HashAlgorithm.crc32`). Use the `hash` extern function to compute the hash of packets 5-tuple (src ip, dst ip, src port, dst port, protocol). The signature of a hash function is:
...
`ipv4` and excluding the `hdrChecksum` field, since its what we are computing here. Set the boolean `condition` to `True` to update the field value.
